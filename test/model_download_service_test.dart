...
void main() {
  group('exact installed artifact protection', () {
    ...
    setUp(() async {
      temporaryDirectory =
          await Directory.systemTemp.createTemp('local_ai_lab_download_test_');
    ...
    test('accepts a matching existing artifact without opening the network',
        () async {
      final List<int> bytes = <int>[1, 2, 3, 4, 5];
      final File target =
          File(p.join(temporaryDirectory.path, 'candidate.gguf'));
